...
- For better response time on Vercel:
  - `PERSIST_ACTIVITY_LOG=false` (default on Vercel in current code)
  - `PERSIST_PRESENCE=false` (default on Vercel in current code)
- Important: `data/db.json` is file storage and not durable across all serverless instances.
  Use an external database for fully reliable persistence in production.
