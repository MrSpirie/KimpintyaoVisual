...
const ADMIN_BAN_DEFAULT_MINUTES = 60;
...
async function loadAdminCoreData() {
  const [overview, usersResponse, rolesResponse] = await Promise.all([
...
async function loadAdminActivityData(options = {}) {
  ...
  try {
    const activityResponse = await api("/api/admin/activity?limit=120");
    ...
    ui.adminActivityBody.innerHTML = `
      <tr>
        <td colspan="4">${escapeHtml(error.message || "Не удалось загрузить активность.")}</td>
      </tr>
    `;
  }
}

async function loadAdminPageData(options = {}) {
  const includeActivity = options.includeActivity !== false;
  const skipLoadedStatus = Boolean(options.skipLoadedStatus);
...
function bindAdminHandlers() {
  ...
  if (ui.adminUsersBody) {
    ui.adminUsersBody.addEventListener("click", async (event) => {
      ...
      const target = targetNode instanceof Element
        ? targetNode
        : (targetNode instanceof Node ? targetNode.parentElement : null);
      if (!(target instanceof Element)) {
        return;
      }

      const actionTarget = target.closest("[data-role-save], [data-user-delete], [data-user-ban], [data-user-unban]");
      if (!(actionTarget instanceof HTMLElement)) {
        return;
      }
      if ("disabled" in actionTarget && actionTarget.disabled) {
        return;
      }

      const userId = actionTarget.getAttribute("data-role-save");
      const deleteId = actionTarget.getAttribute("data-user-delete");
      const banId = actionTarget.getAttribute("data-user-ban");
      const unbanId = actionTarget.getAttribute("data-user-unban");
      ...
      try {
        if (userId) {
          ...
          const targetUser = adminUsersCache.find((item) => Number(item.id) === Number(userId));
          const targetRole = adminRolesCache.find((item) => String(item.name || "").toLowerCase() === nextRole);
          if (targetUser) {
            targetUser.role = nextRole || targetUser.role;
            targetUser.roleLabel = targetRole?.label || nextRole || targetUser.roleLabel;
            targetUser.roleStyle = targetRole
              ? { primary: targetRole.color_primary, secondary: targetRole.color_secondary }
              : targetUser.roleStyle;
            renderAdminUsers(adminUsersCache);
          }
          void loadAdminCoreData().catch(() => {});
        ...
        if (deleteId) {
          ...
          adminUsersCache = adminUsersCache.filter((item) => Number(item.id) !== Number(deleteId));
          renderAdminUsers(adminUsersCache);
          void loadAdminCoreData().catch(() => {});
        ...
        if (banId) {
          const minutes = ADMIN_BAN_DEFAULT_MINUTES;
          const reason = "";
          ...
          const targetUser = adminUsersCache.find((item) => Number(item.id) === Number(banId));
          if (targetUser) {
          ...
          void loadAdminCoreData().catch(() => {});
          setStatus(pickLocalized(ADMIN_STATUS_TEXT.userBanned, "User banned.") + ` (${minutes}m)`);
          return;
        }
        ...
        if (unbanId) {
          ...
          const targetUser = adminUsersCache.find((item) => Number(item.id) === Number(unbanId));
          if (targetUser) {
  ...
  if (ui.adminRolesList) {
    ui.adminRolesList.addEventListener("click", async (event) => {
      ...
      const actionTarget = target.closest("[data-role-delete]");
      if (!(actionTarget instanceof HTMLElement)) {
        return;
      }
      if ("disabled" in actionTarget && actionTarget.disabled) {
      ...
      const roleName = actionTarget.getAttribute("data-role-delete");
      if (!roleName) {
        return;
      }
  ...
  if (ui.adminActivityRefreshBtn) {
    ui.adminActivityRefreshBtn.addEventListener("click", async () => {
      try {
        await loadAdminActivityData({ throwOnError: true });
      } catch (error) {
        setStatus(error.message, true);
      }
    });
