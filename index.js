...
const SESSION_DRIVER = String(process.env.SESSION_DRIVER || "").toLowerCase();
const USE_COOKIE_SESSION = IS_VERCEL || SESSION_DRIVER === "cookie";
const ENABLE_ACTIVITY_LOG = String(process.env.ENABLE_ACTIVITY_LOG || "true").toLowerCase() !== "false";
const PERSIST_ACTIVITY_LOG = String(
  process.env.PERSIST_ACTIVITY_LOG || (IS_VERCEL ? "false" : "true")
).toLowerCase() === "true";
const PERSIST_PRESENCE = String(
  process.env.PERSIST_PRESENCE || (IS_VERCEL ? "false" : "true")
).toLowerCase() === "true";
...
function getDataDirCandidates() {
  ...
  const tempBase = String(process.env.TMPDIR || process.env.TEMP || "/tmp").trim() || "/tmp";

  return [envDataDir, BUNDLED_DATA_DIR, path.join(process.cwd(), "data"), path.join(tempBase, "kimpintyaovisual-data")].filter(Boolean);
}

function isDataDirWritable(dirPath) {
  try {
...
function resolveDataDir() {
  ...
  for (const candidate of candidates) {
    if (isDataDirWritable(candidate)) {
      return candidate;
    }
  }
...
if (dbPath !== BUNDLED_DB_PATH && !fs.existsSync(dbPath) && fs.existsSync(BUNDLED_DB_PATH)) {
  try {
    fs.copyFileSync(BUNDLED_DB_PATH, dbPath);
  } catch (error) {
    console.warn("Cannot copy bundled db.json to runtime data dir:", error.message);
  }
}
...
function writeDbFileSafe(payload) {
  try {
